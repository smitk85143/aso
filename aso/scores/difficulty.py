...
def build(store):
    ...
    def get_title_matches(keyword, apps, lang, country):
        matches = [get_match_type(keyword, store['app'](app, lang, country)['title']) for app in apps]
        counts = {
    ...
    def get_keywords(store):
        def get_keywords(app, lang, country):
            data = store['app'](app, lang, country)
    ...
    def is_competitor(keyword, app, lang, country):
        return keyword in get_keywords(store)(app, lang, country)
    ...
    def get_rating(apps, lang, country):
        avg = sum([store['app'](app, lang, country)['score'] or 0 for app in apps]) / len(apps)
        return {
            'avg': avg,
            'score': avg * 2
        }
    ...
    def get_age(apps, lang, country):
        updated = [get_days_since(store['app'](app, lang, country)['updated']) for app in apps]
    ...
    def get_installs_score(apps, lang, country):
        min_installs = [store['app'](app, lang, country)['minInstalls'] for app in apps]
    ...
    def get_stats(keyword, apps, lang, country):
        ...
        installs = get_installs_score(top_apps, lang, country)
